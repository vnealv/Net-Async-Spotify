...
sub configure_unknown {
    my ($self, %args) = @_;
    for my $k (grep exists $args{$_}, qw(client_id client_secret redirect_uri auth_code access_token refresh_token base_uri)) {
        $self->{$k} = $args{$k};
    }
}
...
sub _add_to_loop {
    my $self = shift;
    $self->add_child($self->{http} = Net::Async::HTTP->new(
            fail_on_error            => 1,
